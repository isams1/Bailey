...
class aged_trial_report(report_sxw.rml_parse):
    ...
    def get_data(self, form):
        ...
        account_ids = account_obj.search(self.cr, self.uid, [('id', '=', form.get('chart_account_id') )])
        for cur_rec in account_obj.browse(self.cr,self.uid,account_ids):
            companyid = cur_rec.company_id.id
        partner_obj = self.pool.get('res.partner')
        account_invoice_obj = self.pool.get('account.invoice')
        account_move_obj = self.pool.get('account.move')
        account_move_line_obj = self.pool.get('account.move.line')
        part_ids = partner_obj.search(self.cr, self.uid, [('id', 'in', form.get('partner_ids'))])
        for partner in partner_obj.browse(self.cr, self.uid, part_ids):
            partner_data = {
                'id': partner.id,
                'name': partner.name,
            ...
            invoice_list = []
            invoice_ids = account_invoice_obj.search(self.cr, self.uid, [('partner_id', '=', partner.id),('company_id','=',companyid)])
            for invoice_data in account_invoice_obj.browse(self.cr, self.uid, invoice_ids):
                res={}   
                if invoice_data.payment_ids:
                    if invoice_data.move_id.line_id:
                        res = {
                        'date': invoice_data.date_invoice,
                        'reference': invoice_data.move_id.name,
                        'debit': invoice_data.amount_total,
                        'credit': invoice_data.amount_total - invoice_data.residual or '0.00',
                        'balance': invoice_data.residual or '0.00'
                        }
                    ...
                    if invoice_data.move_id.line_id:
                        ...
                        'debit': invoice_data.amount_total,
                        'credit': '0.00',
                        'balance': invoice_data.amount_total,
                        }
                        invoice_list.append(res)
            final_result.append({'lines': invoice_list,'partner_data':partner_data})
        return final_result
    ...
    def _get_lines(self, form, partner_id):
        res = []
        partner_obj = self.pool.get('res.partner')
        ...
        partner_ids = partner_obj.search(self.cr, self.uid, [('id', '=', partner_id)])
        partners=[]
        part={}
        for cur_rec in partner_obj.browse(self.cr,self.uid,partner_ids):
            part = {'id':cur_rec.id,
                     'name':cur_rec.name
                     }
        ...
        final_res = []
        for res_data in res:
            add = res_data['1'] + res_data['0']
            res_data.update({'1':add})
        final_res.append(res_data)
        return final_res

report_sxw.report_sxw('report.account.aged_trial_balance_inherited', 'res.partner',
        'addons/sg_account_report/report/partner_ageing_report.rml',parser=aged_trial_report, header=False)
