...
class acc_report_new_emp(models.TransientModel):
    ...
    _defaults = {
         'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'acc.report.new.emp', context=c)
    }
    ...
    def print_report(self):
        ...
        emps = emp_obj.search([('accommodated', '=', True), ('join_date', '>=', self.start_month.date_start), ('join_date', '<=', self.end_month.date_stop)], order='join_date')
        month_list = []
        check_months = []
        for emp in emps:
            ctr = 1
...
#                 'report_name': 'acc.report.new.emp.doc',
#                 'datas': datas
#                 }
        return {
...
class acc_report_new_emp_standard_export(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        res = super(acc_report_new_emp_standard_export, self).default_get(fields)
        res.update({'name': 'New Employee.xls'})
